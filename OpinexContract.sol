...
contract Opinex {
    ...
    function addQuestionManually(
        ...
        require(bytes(questionText).length > 0, "Empty question");
        ...
        emit NewQuestion(questions.length - 1, topic, questionText, options);
    }
        ...
        payable
    {
        ...
        //     // Debug: Log inputs
        //     console.log("Comparing option: %s", option);
        //     console.log("With stored option: %s", q.options[i]);

        //     if (
        //         keccak256(abi.encode(q.options[i])) ==
        //         keccak256(abi.encode(option))
        //     ) {
        //         isValidOption = true;
        //         console.log("Match found at index: %s", i);
        //         break;
        //     }
        // }
        for (uint256 i = 0; i < q.options.length; i++) {
